...
import type { DocsTree, DocsTreeNode } from "./types";

function findTreeNode(tree: DocsTree, slug: string): DocsTreeNode | null {
...
function getVisibleNodes(nodes: DocsTreeNode[]): DocsTreeNode[] {
  return nodes.filter((n) => !n.frontmatter.hidden);
}

function getAllNodes(tree: DocsTree): DocsTreeNode[] {
...
function getDocSourcePath(suiteId: string, version: string, slug: string): string {
  return `/content/docs/${suiteId}/${version}/${slug}.mdx`;
}
