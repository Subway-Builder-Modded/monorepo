...
describe("docs config ownership and architecture", () => {
  it("keeps docs config in src/config/docs only", () => {
    const docsConfigDir = workspacePath("src", "config", "docs");
    const oldDocsConfigDir = workspacePath("src", "features", "docs", "config");
  ...
  it("keeps homepage docs config in suite config modules and sources shared identity centrally", async () => {
    const railyardConfig = workspacePath("src", "config", "railyard", "docs.ts");
    const registryConfig = workspacePath("src", "config", "registry", "docs.ts");
    const templateModConfig = workspacePath("src", "config", "template-mod", "docs.ts");
    ...
    const { getSuiteDocsNavItem } = await import("@/config/site-navigation");
    ...
    const { DOCS_HOMEPAGE_TITLE, DOCS_HOMEPAGE_ICON } = await import("@/config/docs/shared");
  ...
  it("has no legacy homepage data config left under features/home/data", () => {
    const legacyPath = workspacePath("src", "features", "home", "data", "docs");
    expect(fs.existsSync(legacyPath)).toBe(false);
  });

  it("keeps docs config-layer types independent from feature-layer imports", () => {
    const validationPath = workspacePath("src", "config", "docs", "content-validation.ts");
