...
async def _get_author_details_cache() -> dict[str, registry_types.AuthorDetails]:
    global _author_details_cache

    if _author_details_cache is not None:
        return _author_details_cache

    author_index_path = os.path.join(get_repository_path(), "authors", "index.json")
    content = await files.read_and_validate_schema(author_index_path, registry_types.AuthorIndex)
    ...
    _author_details_cache = author_details_cache
    return _author_details_cache


async def get_author_info(author_id: str) -> registry_types.AuthorDetails:
    try:
        author_details_cache = await _get_author_details_cache()
...
def continuous_registry_update():
    if os.path.exists(get_repository_path()):
        logger.info("Already found registry repository")
        repo = git.Repo(get_repository_path())
        while True:
        ...
        repo = git.Repo.clone_from(
            "https://github.com/Subway-Builder-Modded/registry.git", get_repository_path(), multi_options=["--depth=1"]
        )
        while True:
            logger.info("Pulling latest changes from registry repository")
            repo.remotes.origin.pull()
...
async def get_asset_manifest(asset_id: str, asset_type: str) -> registry_types.AssetManifest:
    manifest_path = os.path.join(get_repository_path(), f"{asset_type}s", f"{asset_id}", "manifest.json")
    try:
    ...
    try:
        author_details = await get_author_info(manifest_data.author)
    except FileNotFoundError:
    ...
    common_manifest_fields = {
        ...
        "update": _build_update_config(manifest_data),
        ...
        if asset_type == "map":
            map_manifest_fields = {
                **common_manifest_fields,
                ...
                "initial_view_state": _build_initial_view_state(manifest_data),
