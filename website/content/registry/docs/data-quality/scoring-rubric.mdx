...
description: The full rubric reviewers use to score a map's underlying data, for map authors and maintainers.
icon: Scale
---

{/* Mirrored from the registry's docs/data-quality.md (canonical). Update this page whenever the source changes (rubric_version bumps). */}
...
### Choosing the Grain for Hybrid or Ambiguous Sources {#hybrid-grain}
...
### Multi-Nation Maps {#multi-nation-maps}
...
### Spatial Resolution (R) — Where Mass Can Land {#spatial-resolution}
...
### Intensity Fidelity (I) — How the Total Is Shared Among Those Locations {#intensity-fidelity}
...
_The most important pillar, accounting for half of the weighted score. Scored as **count** (below) × **dasymetric** (the shared [Dasymetric](#dasymetric) R × I ladders)._

### Workplace Count Ground Truth {#workplace-count}
...
Scored via the shared **[Dasymetric](#dasymetric)** ladders (R × I). Resident-specific: intensity is per-building **dwelling** counts or residential floor area (not per-class NACE) — see the _Resident_ rows in [Dasymetric](#dasymetric).

## O/D Shaping {#od-shaping}

_The lightest pillar (weight 0.15) — see [Scoring Criteria](#scoring-criteria) for why._
...
### Worked Examples by Tier {#worked-examples}
...
### Special Demand — A Separate Layer Outside the Normal Modeled Flow {#special-demand}
...
### Data Vintage & Temporal Coherence {#data-vintage}
...
### Point-Placement Detail — Distinct from Input Quality {#point-placement-detail}
