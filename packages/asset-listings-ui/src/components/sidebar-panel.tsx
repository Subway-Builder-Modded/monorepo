...
function getDefaultNavbarOffsetPx(): number {
  return (
    parseFloat(
...
export interface SidebarPanelProps<TFilters = unknown> {
  open: boolean;
  onToggle: () => void;
  ariaLabel: string;
  filters: TFilters;
  ...
  getNavbarOffsetPx?: () => number;
  getMainElement?: () => HTMLElement | null;
  getFooterElement?: () => HTMLElement | null;
  getPositionScrollTarget?: () => EventTarget | null;
  scrollToTop?: () => void;
  scrollClassName?: string;
}

export function SidebarPanel<TFilters = unknown>({
  open,
  onToggle,
  ariaLabel,
  filters,
  children,
  collapsedContent,
  getNavbarOffsetPx = getDefaultNavbarOffsetPx,
  getMainElement = () => document.querySelector<HTMLElement>('main'),
  getFooterElement = () => document.querySelector<HTMLElement>('footer'),
  getPositionScrollTarget = () => window,
  scrollToTop = () => window.scrollTo({ top: 0, behavior: 'auto' }),
  scrollClassName,
}: SidebarPanelProps<TFilters>) {
  ...
  useLayoutEffect(() => {
    ...
    const footerEl = getFooterElement();
    ...
    const scrollTarget = getPositionScrollTarget();
    ...
    return () => {
      ...
      scrollTarget?.removeEventListener?.('scroll', updatePosition as EventListener);
    };
  }, [
    getFooterElement,
    getMainElement,
    getNavbarOffsetPx,
    getPositionScrollTarget,
  ]);

  const lastFiltersRef = useRef<TFilters | null>(null);
  useEffect(() => {
    if (!open) return;
      ...
      >
        ...
        <div className="group/sidebar relative flex min-h-0 flex-1 flex-col">
          <div
            ref={scrollRef}
            className={cn(
              'min-h-0 flex-1 overflow-y-auto overflow-x-clip px-[clamp(0.65rem,1.4vw,1rem)] py-3',
              scrollClassName,
            )}
