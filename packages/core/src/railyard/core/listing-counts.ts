...
export function buildAssetListingCounts(
  ...
  return {
    modTagCounts: buildListingCounts({
      valuesByItem: mods.map((item) => item.tags ?? []),
    }),
    mapLocationCounts: buildListingCounts({
      valuesByItem: maps.map((item) => [item.location]),
      dedupePerItem: false,
    }),
    mapSourceQualityCounts: buildListingCounts({
      valuesByItem: maps.map((item) => [item.source_quality]),
      dedupePerItem: false,
    }),
    mapLevelOfDetailCounts: buildListingCounts({
      valuesByItem: maps.map((item) => [item.level_of_detail]),
      dedupePerItem: false,
    }),
    mapSpecialDemandCounts: buildListingCounts({
