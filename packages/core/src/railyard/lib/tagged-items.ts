import type { SortDirection, SortState } from '@sbm/core/railyard/lib/constants';
...
export function compareItems(
  ...
  switch (sort.field) {
    ...
    case 'city_code': {
      const cityCodeA =
        a.type === 'map' ? ((a.item as MapManifest).city_code ?? '') : '';
      const cityCodeB =
        b.type === 'map' ? ((b.item as MapManifest).city_code ?? '') : '';
      return compareText(cityCodeA, cityCodeB, sort.direction);
    }
    case 'country': {
      const countryA =
        a.type === 'map' ? ((a.item as MapManifest).country ?? '') : '';
      const countryB =
        b.type === 'map' ? ((b.item as MapManifest).country ?? '') : '';
    ...
    case 'population': {
      const popA =
        a.type === 'map' ? ((a.item as MapManifest).population ?? 0) : 0;
      const popB =
        b.type === 'map' ? ((b.item as MapManifest).population ?? 0) : 0;
