...
export interface SidebarPanelProps {
  ...
  ariaLabel: string;
  /** Used to detect when filters change and scroll back to top. */
  filters: object;
  children: ReactNode;
...
export function SidebarPanel({
  ...
  const lastFiltersRef = useRef<object | null>(null);
  useEffect(() => {
    if (!open) return;
    ...
    return (
      ...
      >
        <Dialog.Portal>
          ...
          <Dialog.Content
            className={cx(
              'fixed left-0 z-50 h-[calc(100svh-var(--app-navbar-offset,5.5rem))] w-72',
            ...
            style={
              {
                top: 'var(--app-navbar-offset, 5.5rem)',
              } as CSSProperties
            }
            aria-label={ariaLabel}
          >
            <Dialog.Title className="sr-only">Browse filters</Dialog.Title>
