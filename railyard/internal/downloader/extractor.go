...
func extractMap(d *Downloader, filePath string, mapId string, version string, skipConflictCheck bool) types.AssetInstallResponse {
		...
		files.AtomicDirectoryWrite{
			...
			Callback: func(stagingFolder string) error {
				...
				for key, fileStruct := range filesFound {
					// Skip out-of-tree artifacts in the main loop since they are saved in a separate Railyard-managed directory.
					if !fileStruct.Found || files.IsMapArtifactKey(key) {
						continue
					}
	...
	// Append atomic writes for out-of-tree artifact paths
	for _, artifact := range files.MapArtifacts {
		fileStruct, ok := filesFound[artifact.ArchiveKey]
		if !ok || !fileStruct.Found {
			continue
		}

		destinationPath := d.mapArtifactPath(artifact, configData.Code)
		stagedPath, err := files.StageArchiveForAtomicWrite(destinationPath, fileStruct.FileObject, false)
		if err != nil {
			return d.installError(types.AssetTypeMap, mapId, version, configData, types.InstallErrorExtractFailed, "Failed to stage map artifact", err, "file_path", filePath, "artifact_key", artifact.ArchiveKey)
		}
	...
	// Best-effort removal of artifacts a previous version installed but the new
	// archive no longer carries — they no longer match the just-replaced map data.
	for _, artifact := range files.MapArtifacts {
		if fileStruct, ok := filesFound[artifact.ArchiveKey]; ok && fileStruct.Found {
			continue
		}
		_ = os.Remove(d.mapArtifactPath(artifact, configData.Code))
	}

	if fileStruct, ok := filesFound[files.MapArchiveKeyThumbnail]; !ok || !fileStruct.Found {
		thumbnailPath := paths.JoinLocalPath(d.getMapThumbnailPath(), configData.Code+files.MapThumbnailFileExt)

		if !utils.CanGenerateThumbnail(configData) {
