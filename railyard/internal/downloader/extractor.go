...
func extractMap(d *Downloader, filePath string, mapId string, version string, skipConflictCheck bool) types.AssetInstallResponse {
	...
	if fileStruct, ok := filesFound[files.MapArchiveKeyThumbnail]; !ok || !fileStruct.Found {
		// The tiles were just replaced, so a thumbnail from a previous version no longer
		// matches the installed data. Best-effort removal: a survivor is regenerated by the
		// launch-time staleness check in generateMissingThumbnails.
