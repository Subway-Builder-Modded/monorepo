...
func (r *Registry) refreshRepo() error {
	...
	// Legacy clones (made before tags were excluded) carry the registry's tags and the full old
	// snapshots they pin, which can ~triple the on-disk .git. A clean tag-free re-clone sheds them.
	if repoHasTags(repo) {
...
// repoHasTags reports whether the local clone has any tag refs. Tag-free clones (the current default)
// never do, so a tag indicates a legacy clone that should be re-cloned to reclaim space.
		...
		files.AtomicDirectoryWrite{
			...
			Callback: func(stagingPath string) error {
				repo, cloneErr := git.PlainClone(stagingPath, false, &git.CloneOptions{
					...
					Depth:         1,
					NoCheckout:    true,
					// Exclude tags. go-git defaults to AllTags, which drags every tagged commit's full
					// snapshot into the shallow pack (e.g. v0.1.0 ~tripled .git: 140MB -> 400MB). The
					// client only ever needs main@HEAD.
					Tags:     git.NoTags,
					Progress: progress,
				})
				if cloneErr != nil {
				...
				hash, wt, err := prepareCheckout(repo)
				if err != nil {
					return err
				}
...
// prepareCheckout resolves the hash of the remote and acquires the worktree.
func prepareCheckout(repo *git.Repository) (plumbing.Hash, *git.Worktree, error) {
...
func (r *Registry) areSparseSubtreesUnchanged(repo *git.Repository, newCommitHash plumbing.Hash) bool {
	head, err := repo.Head()
	if err != nil {
		return false
	}
